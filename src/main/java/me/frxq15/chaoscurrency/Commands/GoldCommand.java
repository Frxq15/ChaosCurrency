...
public class GoldCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        ...
        if(!p.hasPermission("chaosucurrency.gold")) {
            p.sendMessage(ChaosCurrency.formatMsg("NO_PERMISSION"));
            return true;
        }
        if(strings.length == 0) {
            PlayerData pd = PlayerData.getPlayerData(plugin, p.getUniqueId());
            p.sendMessage(ChaosCurrency.formatMsg("GOLD_BALANCE").replace("%amount%", pd.getGold()+""));
            return true;
        }
        if(strings.length == 1) {
            ...
            if(!target.isOnline()) {
                ...
                p.sendMessage(ChaosCurrency.formatMsg("GOLD_BALANCE_OTHER").replace("%amount%", pd.getGold()+"").replace("%player%", target.getName()));
        ...
        p.sendMessage(ChaosCurrency.colourize("&cUsage: /gold <player>"));
        return true;
    }
}
