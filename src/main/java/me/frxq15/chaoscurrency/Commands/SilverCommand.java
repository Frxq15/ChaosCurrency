...
public class SilverCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        ...
        if(!p.hasPermission("chaosucurrency.silver")) {
            p.sendMessage(ChaosCurrency.formatMsg("NO_PERMISSION"));
            return true;
        }
        if(strings.length == 0) {
            PlayerData pd = PlayerData.getPlayerData(plugin, p.getUniqueId());
            p.sendMessage(ChaosCurrency.formatMsg("SILVER_BALANCE").replace("%amount%", pd.getGold()+""));
            return true;
        }
        if(strings.length == 1) {
            ...
            if(!target.isOnline()) {
                ...
                p.sendMessage(ChaosCurrency.formatMsg("SILVER_BALANCE_OTHER").replace("%amount%", pd.getSilver()+"").replace("%player%", target.getName()));
        ...
        p.sendMessage(ChaosCurrency.colourize("&cUsage: /silver <player>"));
        return true;
    }
}
